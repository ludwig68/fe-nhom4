    ...
    <div class="grid grid-cols-1 gap-8 lg:grid-cols-4">
      ...
      <div class="lg:col-span-3">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div v-for="room in rooms" :key="room.roomId" class="flex flex-col rounded-lg border border-stone-200 bg-white p-5 shadow-sm transition hover:shadow-md">
            <div class="mb-2 flex items-start justify-between">
              <div>
                <h3 class="text-lg font-bold text-stone-900">Phòng {{ room.roomNumber }}</h3>
                <p class="text-sm text-stone-500">{{ room.branchName }}</p>
              </div>
              <span class="rounded-full px-2.5 py-1 text-xs font-semibold" :class="room.status === 'Available' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'">
                {{ room.status }}
              </span>
            </div>
